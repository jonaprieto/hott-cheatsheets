%  Based on `macros.tex` from the sources of HoTT's book.

% \NeedsTeXFormat{LaTeX2e}
% \ProvidesClass{hott}[2018/06/01 LaTeX class]
...
%%% Code/encode/decode
\ifdefined\code
  \newcommand{\mcode}{\ensuremath{\mathsf{code}}\xspace}
\else
...
\else
  \newcommand{\fact}{\ensuremath{\mathsf{code}}\xspace}
\fi
...
\newtheorem{ex}{Exer.}[chapter]
\crefname{ex}{Exer.}{Exercises}
...
\Crefrangemultiformat{section}{Sect.~#3#1#4--#5#2#6}{ and~#3#1#4--#5#2#6}{, #3#1#4--#5#2#6}{ and~#3#1#4--#5#2#6}
...
%% Use \symlabel instead of \fakelabel to mark a pageref that you need in the index of symbols
\newcounter{symindex}
\newcommand{\symlabel}[1]{\refstepcounter{symindex}\fakelabel{#1}}
