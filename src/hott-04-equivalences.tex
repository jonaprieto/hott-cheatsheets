...
\begin{enumerate}
\item $\qinv(f) \to \isequiv (f)$.\fakelabel{item:beb1}
\item $\isequiv (f) \to \qinv(f)$.\fakelabel{item:beb2}
\item $\isequiv(f)$ is a mere proposition.\fakelabel{item:beb3}
\end{enumerate}
...
\begin{lem}\fakelabel{lem:qinv-autohtpy}
...
\begin{lem}\fakelabel{lem:autohtpy}
  Suppose we have a type $A$ with $a:A$ and $q:a=a$ such that
  \begin{enumerate}
  \item The type $a=a$ is a set.\fakelabel{item:autohtpy1}
  \item For all $x:A$ we have $\brck{a=x}$.\fakelabel{item:autohtpy2}
  \item For all $p:a=a$ we have $p\ct q = q \ct p$.\fakelabel{item:autohtpy3}
  \end{enumerate}
...
\begin{thm}\fakelabel{thm:qinv-notprop}
...
\begin{defn}\fakelabel{defn:ishae}
...
\begin{lem}\fakelabel{lem:coh-equiv}
...
\begin{thm}\fakelabel{thm:equiv-iso-adj}
...
\begin{defn}\fakelabel{defn:homotopy-fiber}
  The \define{fiber}
  \indexdef{fiber}%
...
\begin{defn}\fakelabel{defn:linv-rinv}
...
\begin{lem}\fakelabel{thm:equiv-compose-equiv}
...
\begin{lem}\fakelabel{lem:inv-hprop}
...
\begin{defn}\fakelabel{defn:lcoh-rcoh}
...
\begin{lem}\fakelabel{lem:coh-hfib}
...
\begin{lem}\fakelabel{lem:coh-hprop}
...
\begin{thm}\fakelabel{thm:hae-hprop}
...
\begin{defn}\fakelabel{defn:biinv}
...
\begin{thm}\fakelabel{thm:isprop-biinv}
...
\begin{cor}\fakelabel{thm:equiv-biinv-isequiv}
...
\begin{defn}[Contractible maps] \fakelabel{defn:equivalence}
...
% \begin{align}
%   \iscontr(f) &\defeq \prd{y:B} \iscontr(\hfib f y)\fakelabel{eq:iscontrf}
%   % \\
...
\begin{thm}\fakelabel{thm:lequiv-contr-hae}
...
\begin{lem}\fakelabel{thm:contr-hprop}
...
\begin{thm}\fakelabel{thm:equiv-contr-hae}
...
\begin{cor}\fakelabel{thm:equiv-inhabcod}
...
\fakelabel{sec:concluding-remarks}
...
\begin{defn}\fakelabel{defn:surj-emb}
  Let $f:A\to B$.
  \begin{enumerate}
...
\begin{equation}
  \prd{x,y:A} (\id[B]{f(x)}{f(y)}) \to (\id[A]xy).\fakelabel{eq:injective}
\end{equation}
...
\begin{thm}\fakelabel{thm:mono-surj-equiv}
...
\fakelabel{sec:equiv-closures}
\fakelabel{sec:fiberwise-equivalences}
\index{equivalence!properties of}%
...
\begin{thm}[The 2-out-of-3 property]\fakelabel{thm:two-out-of-three}
...
\begin{defn}\fakelabel{defn:retract}
...
\begin{lem}\fakelabel{lem:func_retract_to_fiber_retract}
...
\begin{thm}\fakelabel{thm:retract-equiv}
...
\begin{defn}\fakelabel{defn:total-map}
...
\begin{thm}\fakelabel{fibwise-fiber-total-fiber-equiv}
...
%   \tag{$*$}\fakelabel{eq:uses-sum-over-paths} \\
%   & \jdeq \hfib{f(x)}{v}. \notag
% \end{align}
...
\begin{thm}\fakelabel{thm:total-fiber-equiv}
...
\fakelabel{sec:object-classification}
...
\begin{lem}\fakelabel{thm:fiber-of-a-fibration}
...
\begin{lem}\fakelabel{thm:total-space-of-the-fibers}
...
\begin{thm}\fakelabel{thm:nobject-classifier-appetizer}
...
\begin{thm}\fakelabel{thm:object-classifier}
...
\fakelabel{sec:univalence-implies-funext}
...
\begin{defn}\fakelabel{weakfunext}
...
\begin{lem} \fakelabel{UA-eqv-hom-eqv}
...
\begin{cor}\fakelabel{contrfamtotalpostcompequiv}
...
\begin{ex}\fakelabel{ex:two-sided-adjoint-equivalences}
...
\begin{ex}\fakelabel{ex:symmetric-equiv}
...
\begin{ex} \fakelabel{ex:qinv-autohtpy-no-univalence}
  Reformulate the proof of \cref{lem:qinv-autohtpy} without using univalence.
\end{ex}

\begin{ex}[The unstable octahedral axiom]\fakelabel{ex:unstable-octahedron}
...
\begin{ex}\fakelabel{ex:2-out-of-6}
...
\begin{ex}\fakelabel{ex:qinv-univalence}
  For $A,B:\UU$, define
...
\begin{ex}\fakelabel{ex:embedding-cancellable}
  Show that a function $f:A\to B$ is an embedding if and only if the following two conditions hold:
  \begin{enumerate}
  \item $f$ is \emph{left cancellable}, i.e.\ for any $x,y:A$, if $f(x)=f(y)$ then $x=y$.\fakelabel{item:ex:ec1}
  \item For any $x:A$, the map $\apfunc f: \Omega(A,x) \to \Omega(B,f(x))$ is an equivalence.\fakelabel{item:ex:ec2}
  \end{enumerate}
...
\begin{ex}\fakelabel{ex:cancellable-from-bool}
