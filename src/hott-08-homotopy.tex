...
% }
% \fakelabel{tab:homotopy-groups-of-spheres}
% \end{table}
% \egroup
% 
% 
...
% 
\begin{defn}[Homotopy Groups]\fakelabel{def-of-homotopy-groups}
...
% \fakelabel{sec:pi1s1-initial-thoughts}
% 
...
% \fakelabel{sec:pi1s1-classical-proof}
% 
% \index{classical!homotopy theory|(}%
...
%   \end{tikzpicture}
%   \caption{The winding map in classical topology}\fakelabel{fig:winding}
% \end{figure}
% 
...
% \fakelabel{sec:pi1s1-universal-cover}
% 
% \index{universal!cover|(}%
% \index{cover!universal|(}%
% \index{covering space!universal|(}%
...
% \index{integers}
% 
\begin{defn}[Universal Cover of $\Sn^1$] \fakelabel{S1-universal-cover}
...
\begin{lem} \fakelabel{lem:transport-s1-code}
...
% \begin{align}
%   \prd{x:\Sn^1} &((\base=x) \to \code(x)) \qquad\text{and/or} \fakelabel{eq:pi1s1-encode}\\
%   \prd{x:\Sn^1} &(\code(x) \to (\base=x))\fakelabel{eq:pi1s1-decode}
% \end{align}
...
% \fakelabel{subsec:pi1s1-encode-decode}
% 
...
% 
\begin{defn}\fakelabel{thm:pi1s1-decode}
...
% 
\begin{lem} \fakelabel{lem:s1-decode-encode}
...
% 
\begin{lem} \fakelabel{lem:s1-encode-decode} For all
...
\begin{cor}\fakelabel{cor:omega-s1}
...
% \fakelabel{subsec:pi1s1-homotopy-theory}
% 
...
% 
\begin{lem}\fakelabel{thm:iscontr-s1cover}
...
% \end{proof}
% 
\begin{cor}\fakelabel{thm:encode-total-equiv}
...
% 
\begin{thm} \fakelabel{thm:suspension-increases-connectedness}
...
% \end{proof}
% 
\begin{cor} \fakelabel{cor:sn-connected}
...
% \end{proof}
% 
\begin{lem} \fakelabel{lem:pik-nconnected}
...
% \fakelabel{sec:long-exact-sequence-homotopy-groups}
% 
...
% 
\begin{defn}\fakelabel{def:pointedmap}
...
% 
\begin{defn}\fakelabel{def:loopfunctor}
...
% 
\begin{lem}\fakelabel{thm:fiber-of-the-fiber}
  Let $f:X\to Y$ be a pointed map of pointed spaces.  Then:
  \begin{enumerate}
  \item The fiber of $f^{(1)}\defeq \proj1 : \hfib f {y_0} \to X$ is equivalent to $\Omega Y$.\fakelabel{item:fibseq1}
  \item Similarly, the fiber of $f^{(2)} : \Omega Y \to \hfib f {y_0}$ is equivalent to $\Omega X$.\fakelabel{item:fibseq2}
  \item Under these equivalences, the pointed map $f^{(3)} : \Omega X\to \Omega Y$ is identified with the pointed map $\Omega f\circ\rev{(\blank)}$.\fakelabel{item:fibseq3}
  \end{enumerate}
\end{lem}
% \begin{proof}
...
\begin{lem}\fakelabel{thm:ses}
  Suppose given an exact sequence of abelian groups:
  %
  \[\xymatrix{K \ar[r]& G \ar^f[r] & H \ar[r] & Q.}\]
  %
  \begin{enumerate}
  \item If $K=0$, then $f$ is injective.\fakelabel{item:sesinj}
  \item If $Q=0$, then $f$ is surjective.\fakelabel{item:sessurj}
  \item If $K=Q=0$, then $f$ is an isomorphism.\fakelabel{item:sesiso}
  \end{enumerate}
\end{lem}
% \begin{proof}
...
% 
\begin{cor}\fakelabel{thm:conn-pik}
  Let $f:A\to B$ be $n$-connected and $a:A$, and define $b\defeq f(a)$.  Then:
  \begin{enumerate}
  \item If $k\le n$, then $\pi_k(f):\pi_k(A,a) \to \pi_k(B,b)$ is an isomorphism.\fakelabel{item:conn-pik1}
  \item If $k=n+1$, then $\pi_k(f):\pi_k(A,a) \to \pi_k(B,b)$ is surjective.\fakelabel{item:conn-pik2}
  \end{enumerate}
\end{cor}
% \begin{proof}
...
% 
\begin{thm}[Hopf Fibration]\fakelabel{thm:hopf-fibration}
...
% 
\begin{cor} \fakelabel{cor:pis2-hopf}
...
% \fakelabel{sec:fib-over-pushout}
% 
% We first start with a lemma explaining how to construct fibrations over
% pushouts.
% \index{pushout}%
% 
\begin{lem}\fakelabel{lem:fibration-over-pushout}
...
% We have proven:
% 
\begin{lem}\fakelabel{lem:hopf-construction}
...
% equivalent to $\Sn^3$.
% 
\begin{lem}\fakelabel{lem:hspace-S1}
...
% 
\begin{lem}\fakelabel{thm:conn-trunc-variable-ind}
...
% 
\begin{lem}[Wedge connectivity lemma]\fakelabel{thm:wedge-connectivity}
...
% 
\begin{thm}[The Freudenthal suspension theorem]\fakelabel{thm:freudenthal}
...
% 
\begin{defn}\fakelabel{thm:freudcode}
...
%   \begin{equation}\fakelabel{eq:freudmap}
...
% 
\begin{lem}\fakelabel{thm:freudlemma}
...
%   =\tproj{2n}{(x_1,r')}\fakelabel{eq:freudcompute1}
% \end{equation}
...
%   = \tproj{2n}{(x_1,r'')}\fakelabel{eq:freudcompute2}
% \end{equation}
...
%     \;=\;
%     c\left(\north,\refl{\merid(x_1) \ct \opp{\merid(x_0)}}\right).\fakelabel{eq:freudgoal}
%   \end{equation}
...
% 
\begin{cor}[Freudenthal Equivalence] \fakelabel{cor:freudenthal-equiv}
Suppose that $X$ is $n$-connected and pointed, with $n\geq 0$.
...
% 
\begin{cor}[Stability for Spheres] \fakelabel{cor:stability-spheres}
...
% \begin{equation}\fakelabel{eq:freudenthal-for-spheres}
...
%   \begin{equation}
%     \code(ib, ifa) \eqvsym \code(ib,jga).\fakelabel{eq:bfa-bga}
%   \end{equation}
...
%   \begin{equation}\fakelabel{eq:bfa-bga-comm}
%   \vcenter{\xymatrix{
...
% 
% We will now prove:
\begin{thm}[Naive van Kampen theorem]\fakelabel{thm:naive-van-kampen}
...
% \begin{narrowmultline}\fakelabel{eq:decode}
...
% \index{fundamental!group|(}%
% 
\begin{eg}\fakelabel{eg:suspension}
...
% \fakelabel{sec:better-vankampen}
% 
...
% \item For $a:A$ and $b:B$, we require an equivalence
%   \begin{equation}
%     \code(ib, ifa) \eqvsym \code(ib,jga).\fakelabel{eq:bfa-bga2}
%   \end{equation}
...
% \end{itemize}
% 
\begin{thm}[van Kampen with a set of basepoints]\fakelabel{thm:van-Kampen}
  For all $u,v:P$ there is an equivalence
  \[ \Pi_1P(u,v) \eqvsym \code(u,v). \]
...
% 
\begin{thm}\fakelabel{thm:whitehead0}
  Suppose $f:A\to B$ is a function such that
  \begin{enumerate}
  \item $\trunc0 f : \trunc0 A \to \trunc0 B$ is surjective, and\fakelabel{item:whitehead01}
  \item for any $x,y:A$, the function $\apfunc f : (\id[A]xy) \to (\id[B]{f(x)}{f(y)})$ is an equivalence.\fakelabel{item:whitehead02}
  \end{enumerate}
...
% 
\begin{cor}\fakelabel{thm:whitehead1}
  Suppose $f:A\to B$ is a function such that
  \begin{enumerate}
...
% 
\begin{thm}\fakelabel{thm:whiteheadn}
  Suppose $A$ and $B$ are $n$-types and $f:A\to B$ is such that
  \begin{enumerate}
  \item $\trunc0f:\trunc0A \to \trunc0B$ is a bijection, and\fakelabel{item:wh0}
  \item $\pi_k(f):\pi_k(A,x) \to \pi_k(B,f(x))$ is a bijection for all $k\ge 1$ and all $x:A$.\fakelabel{item:whk}
  \end{enumerate}
  Then $f$ is an equivalence.
\end{thm}
% 
...
% 
\begin{cor}\fakelabel{thm:whitehead-contr}
...
% 
\begin{cor}\fakelabel{thm:pik-conn}
...
% \fakelabel{sec:general-encode-decode}
% 
...
$\code : A \to \type$, if
\begin{enumerate}
\item $c_0 : \code(a_0)$,\fakelabel{item:ed1}
\item $\decode : \prd{x:A} \code(x) \to (\id{a_0}{x})$,\fakelabel{item:ed2}
\item for all $c : \code(a_0)$, \id{\transfib{\code}{\decode(c)}{c_0}}{c}, and\fakelabel{item:ed3}
\item $\id{\decode(c_0)}{\refl{}}$,\fakelabel{item:ed4}
\end{enumerate}
...
\item \fakelabel{item:decode-encode-loop-iii}
  $\id{\encode_{a_0}(\decode_{a_0}(c))}{c}$ for all $c : \code(a_0)$, and
\item \fakelabel{item:decode-encode-loop-iv}
...
% \index{pushout}%
% 
\begin{thm}[Blakers--Massey theorem]\fakelabel{Blakers-Massey}
...
% 
\begin{thm}[Eilenberg--Mac Lane Spaces]\fakelabel{Eilenberg-Mac-Lane-Spaces}
...
\end{thm}
% 
\begin{thm}[Covering spaces]\fakelabel{thm:covering-spaces}
...
\sectionExercises
% 
\begin{ex}\fakelabel{ex:homotopy-groups-resp-prod}
  Prove that homotopy groups respect products: $\eqv{\pi_n(A\times B)}{\pi_n(A)\times \pi_n(B)}$.
\end{ex}
% 
\begin{ex}\fakelabel{ex:decidable-equality-susp}
...
\end{ex}
% 
\begin{ex}\fakelabel{ex:pointed-equivalences}
...
\end{ex}
% 
\begin{ex}\fakelabel{ex:SuperHopf}
...
\end{ex}
% 
\begin{ex}\fakelabel{ex:vksusppt}
...
\end{ex}
% 
\begin{ex}\fakelabel{ex:vksuspnopt}
