...
% 
\begin{defn}\fakelabel{def:hlevel}
...
% 
\begin{thm}\fakelabel{thm:h-level-retracts}
...
% 
\begin{thm}\fakelabel{thm:hlevel-cumulative}
...
% 
\begin{thm}\fakelabel{thm:hlevel-prod}
 Let $n\geq -2$, and let $A : \type$ and $B : A \to \type$.
...
% 
\begin{thm}\fakelabel{thm:isaprop-isofhlevel}
...
% \end{align*}
% 
\begin{thm}\fakelabel{thm:hleveln-of-hlevelSn}
...
% 
\begin{thm}\fakelabel{thm:h-set-uip-K}
...
% 
\begin{thm}\fakelabel{thm:h-set-refrel-in-paths-sets}
...
% \end{proof}
% 
\begin{cor}\fakelabel{notnotstable-equality-to-set}
...
% 
\begin{lem}\fakelabel{lem:hedberg-helper}
...
% 
% \index{anger}
\begin{thm}[Hedberg]\fakelabel{thm:hedberg}
...
% 
\begin{thm}\fakelabel{prop:nat-is-set}
...
% 
\begin{thm}\fakelabel{thm:hlevel-loops}
...
% 
\begin{lem}\fakelabel{lem:hlevel-if-inhab-hlevel}
...
% 
\begin{thm}\fakelabel{thm:ntype-nloop}
...
% 
\begin{thm}\fakelabel{thm:truncn-ind}
...
% 
\begin{lem}[Universal property of truncations]\fakelabel{thm:trunc-reflective}
...
% \begin{equation}
%   \mathsf{nat}^f_n : \prd{a:A} \trunc n f(\tproj n a) = \tproj n {f(a)},\fakelabel{eq:trunc-nat}
% \end{equation}
...
% 
\begin{lem}\fakelabel{thm:trunc-htpy}
...
% 
\begin{thm}\fakelabel{cor:trunc-prod}
...
% 
\begin{thm}\fakelabel{thm:trunc-in-truncated-sigma}
...
% \end{proof}
% 
\begin{cor}\fakelabel{thm:refl-over-ntype-base}
...
% \begin{equation}
%   f:\ttrunc n{x=_Ay}\to \Big(\tproj {n+1}x=_{\trunc{n+1}A}\tproj {n+1}y\Big)\fakelabel{eq:path-trunc-map}
% \end{equation}
% defined by
...
% 
\begin{thm} \fakelabel{thm:path-truncation}
...
% 
\begin{lem} \fakelabel{lem:truncation-le}
...
% \begin{align}
%   \composecocone{\idfunc[D]}c &= c \fakelabel{eq:composeconeid}\\
%   \composecocone{(u\circ{}t)}c&=\composecocone{u}(\composecocone{t}c). \fakelabel{eq:composeconefunc}
% \end{align}
% 
\begin{defn}
...
% \begin{equation}\fakelabel{eq:mapofspans-htpy}
% \xymatrix{
...
% 
% \begin{lem}\fakelabel{thm:conemap-funct}
...
% \begin{equation}
%   \tprojf[D] n \circ c = \trunc n c \circ \tprojf[\Ddiag]n. \fakelabel{eq:conetrunc}
% \end{equation}
% for any $c:\cocone{\Ddiag}{D}$.
% Now we can prove our desired theorem.
% 
\begin{thm}
  \fakelabel{reflectcommutespushout}
...
% 
\begin{lem}\fakelabel{thm:minusoneconn-surjective}
  \index{function!surjective}%
  A function $f$ is $(-1)$-connected if and only if it is surjective in the sense of mono-surj.%\cref{sec:mono-surj}.
\end{lem}
% \begin{proof}
...
% 
\begin{rmk}\fakelabel{rmk:connectedness-indexing}
...
\end{cor}
% 
% \begin{lem}\fakelabel{lem:nconnected_postcomp}
...
% 
% \begin{lem}\fakelabel{prop:nconnected_tested_by_lv_n_dependent types}
...
% \begin{enumerate}
% \item $f$ is $n$-connected.\fakelabel{item:conntest1}
% \item For every $P:B\to\ntype{n}$, the map $\lam{s} s\circ f$ is an equivalence.\fakelabel{item:conntest2}
% \item For every $P:B\to\ntype{n}$, the map $\lam{s} s\circ f$ has a section.\fakelabel{item:conntest3}
% \end{enumerate}
% \end{lem}
% 
% \begin{proof}
...
% \end{proof}
% 
\begin{cor}\fakelabel{cor:totrunc-is-connected}
...
% 
% \begin{cor}\fakelabel{thm:nconn-to-ntype-const}\fakelabel{connectedtotruncated}
...
% \end{proof}
% 
% \begin{lem}\fakelabel{lem:nconnected_to_leveln_to_equiv}
...
% 
% \begin{lem}\fakelabel{thm:connected-pointed}
%   \index{basepoint}%
...
% % 
% \begin{lem}\fakelabel{lem:nconnected_postcomp_variation}
...
% % 
% \begin{lem}\fakelabel{prop:nconn_fiber_to_total}
...
% % 
% \begin{lem} \fakelabel{lem:connected-map-equiv-truncation}
...
% % \fakelabel{sec:image-factorization}
% % 
...
% % 
% \begin{lem}\fakelabel{thm:modal-mono}
...
%   In particular, $f$ is $(-1)$-truncated if and only if it is an embedding in the sense of mono-surj.%\cref{sec:mono-surj}.
% \end{lem}
% % \begin{proof}
...
% % 
% \begin{defn}\fakelabel{defn:modal-image}
...
% \end{defn}
% % 
% \begin{lem}\fakelabel{prop:to_image_is_connected}
...
% % 
% \begin{lem}\fakelabel{prop:factor_equiv_fiber}
...
% % 
% \begin{thm}\fakelabel{thm:orth-fact}
...
% % 
% \begin{lem}\fakelabel{lem:hfiber_wrt_pullback}
...
% % \end{proof}
% % 
% \begin{thm}\fakelabel{thm:stable-images}
...
% %       }}
% % \end{equation*}
% If the outer rectangle is a pullback, then so is the bottom square (and hence so is the top square, by ex:pullback-pasting). Consequently, images are stable under pullbacks.
% \end{thm}
% % 
% % \begin{proof}
...
% % 
% \begin{defn}\fakelabel{defn:reflective-subuniverse}
...
% % 
% \begin{thm}\fakelabel{thm:reflsubunv-forall}
...
% % 
% \begin{cor}\fakelabel{cor:trunc_prod}
...
%   \begin{enumerate}
%   \item If $A:\P$ and $B:A\to \P$, then $\sm{x:A} B(x)$ is in \P.\fakelabel{item:mchr1}
%   \item for every $A:\type$, type family $B:\reflect A\to\P$, and map $g:\prd{a:A} B(\project(a))$, there exists $f:\prd{z:\reflect A} B(z)$ such that $f(\project(a)) = g(a)$ for all $a:A$.\fakelabel{item:mchr2}
%   \end{enumerate}
% \end{thm}
% % \begin{proof}
...
% % 
% \begin{defn}\fakelabel{defn:modality}
% A \define{modality}
% \indexdef{modality}
% is an operation $\modal:\type\to\type$ for which there are
% \begin{enumerate}
% \item functions $\mreturn^\modal_A:A\to\modal(A)$ for every type $A$.\fakelabel{item:modal1}
% \item for every $A:\type$ and every type family $B:\modal(A)\to\type$, a function\fakelabel{item:modal2}
% \begin{equation*}
% \ind{\modal}:\Parens{\prd{a:A}\modal(B(\mreturn^\modal_A(a)))}\to\prd{z:\modal(A)}\modal(B(z)).
% \end{equation*}
% \item A path $\ind\modal(f)(\mreturn^\modal_A(a)) = f(a)$ for each $f:\prd{a:A}\modal(B(\mreturn^\modal_A(a)))$.\fakelabel{item:modal3}
% \item For any $z,z':\modal(A)$, the function $\mreturn^\modal_{z=z'} : (z=z') \to \modal(z=z')$ is an equivalence.\fakelabel{item:modal4}
% \end{enumerate}
...
% % 
% \begin{thm}\fakelabel{prop:lv_n_deptype_sec_equiv_by_precomp}
...
% % 
% \begin{cor}
%   For any modality $\modal$, the $\modal$-modal types form a reflective subuniverse satisfying the equivalent conditions of \ref{thm:modal-char}.
% \end{cor}
% % 
...
% \sectionExercises
% % 
% % \begin{ex}\fakelabel{ex:all-types-sets}\
% %   \begin{enumerate}
...
% % \end{ex}
% % 
% % \begin{ex}\fakelabel{ex:s2-colim-unit}
...
% % \end{ex}
% % 
% % \begin{ex}\fakelabel{ex:ntypes-closed-under-wtypes}
...
% % \end{ex}
% % 
% % \begin{ex}\fakelabel{ex:connected-pointed-all-section-retraction}
% %   Use \cref{prop:nconn_fiber_to_total} to extend \cref{thm:connected-pointed} to any section-retraction pair.
% % \end{ex}
% % 
% % \begin{ex}\fakelabel{ex:ntype-from-nconn-const}
...
% % \end{ex}
% % 
% % \begin{ex}\fakelabel{ex:connectivity-inductively}
...
% % \end{ex}
% % 
% % \begin{ex}\fakelabel{ex:n-truncation-not-left-exact}
...
% % \end{ex}
% % 
% % \begin{ex}\fakelabel{ex:double-negation-modality}
% %   Show that $X\mapsto (\neg\neg X)$ is a modality.\index{modal!operator}%
% % \end{ex}
% % 
% % \begin{ex}\fakelabel{ex:prop-modalities}
...
% % \end{ex}
% % 
% % \begin{ex}\fakelabel{ex:f-local-type}
...
% % \end{ex}
% % 
% % \begin{ex}\fakelabel{ex:trunc-spokes-no-hub}
...
% % \end{ex}
% % 
% % \begin{ex}\fakelabel{ex:s2-colim-unit-2}
...
% % \end{ex}
% % 
% % \begin{ex}\fakelabel{ex:fiber-map-not-conn}
...
% % \end{ex}
% % 
% % \begin{ex}\fakelabel{ex:is-conn-trunc-functor}
% %   Show that if $f : A \to B$ is $n$-connected, then $\trunc kf : \trunc kA \to \trunc kB$ is also $n$-connected.
% % \end{ex}
% % 
% % \begin{ex}\fakelabel{ex:categorical-connectedness}
