...
%
% \begin{enumerate}
% \item $\uset$ is finitely complete.\fakelabel{item:reg1}
% \item The kernel pair $\proj1,\proj2: (\sm{x,y:A} f(x)= f(y)) \to A$ of any
%   function $f : A \to B$ has a coequalizer.\fakelabel{item:reg2}
%   \indexdef{kernel!pair}
% \item Pullbacks of regular epimorphisms are again regular epimorphisms.\fakelabel{item:reg3}
% \end{enumerate}
%
...
\begin{lem}\fakelabel{thm:mono}
  ...
  \item $f$ is a \define{monomorphism}:
    ...
    for all $x:A$ and ${g,h:\hom_A(x,a)}$, if $f\circ g = f\circ h$ then $g=h$.\fakelabel{item:mono1}
  \item (If $A$ has pullbacks) the diagonal map $a\to a\times_b a$ is an isomorphism.\fakelabel{item:mono4}
  \item For all $x:A$ and $k:\hom_A(x,b)$, the type $\sm{h:\hom_A(x,a)} (k = f\circ h)$ is a mere proposition.\fakelabel{item:mono2}
  \item For all $x:A$ and ${g:\hom_A(x,a)}$, the type $\sm{h:\hom_A(x,a)} (f\circ g = f\circ h)$ is contractible.\fakelabel{item:mono3}
  \end{enumerate}
\end{lem}
% \begin{proof}
...
% \end{proof}
% 
\begin{lem}\fakelabel{thm:inj-mono}
...
% \end{rem}
% 
\begin{thm}\fakelabel{thm:set_regular}\fakelabel{lem:images_are_coequalizers}
...
% \end{proof}
% 
\begin{lem}\fakelabel{lem:pb_of_coeq_is_coeq}
...
% 
% 
\subsection{Quotients}\fakelabel{subsec:quotients}
% 
% \index{set-quotient|(}%
...
% 
\begin{lem}\fakelabel{lem:sets_exact}
...
% 
\begin{thm}\fakelabel{prop:kernels_are_effective}
...
% 
\begin{thm}\fakelabel{thm:settopos}
  \index{propositional!resizing}%
...
% 
\begin{lem}\fakelabel{prop:trunc_of_prop_is_set}
...
% \end{proof}
% 
\begin{thm}[Diaconescu]\fakelabel{thm:1surj_to_surj_to_pem}
...
% 
% \begin{cor}\fakelabel{cor:ACtoLEM0}
...
% \fakelabel{sec:cardinals}
% 
\begin{defn}\fakelabel{defn:card}
...
\end{defn}
% 
\begin{lem}\fakelabel{card:semiring}
...
% 
\begin{lem}\fakelabel{thm:injsurj}
  \index{function!injective}%
  \index{function!surjective}%
  Consider the following statements:
  \begin{enumerate}
  \item There is an injection $A\to B$.\fakelabel{item:cle-inj}
  \item There is a surjection $B\to A$.\fakelabel{item:cle-surj}
  \end{enumerate}
...
% \fakelabel{sec:ordinals}
% 
% \index{ordinal|(}%
% 
\begin{defn}\fakelabel{defn:accessibility}
  Let $A$ be a set and
...
\end{eg}
% 
\begin{eg}\fakelabel{thm:wtype-wf}
...
% \end{proof}
% 
\begin{defn}\fakelabel{def:simulation}
  ...
  \begin{enumerate}
  \item if $a<a'$, then $f(a)<f(a')$, and\fakelabel{item:sim1}
  \item for all $a:A$ and $b:B$, if $b<f(a)$, then there merely exists an $a'<a$ with $f(a')=b$.\fakelabel{item:sim2}
  \end{enumerate}
\end{defn}
% 
\begin{lem}
...
% 
\begin{thm}\fakelabel{thm:ordord}
  $(\ord,<)$ is an ordinal.
\end{thm}
% 
% \noindent
...
% 
\begin{lem}\fakelabel{thm:ordsucc}
...
% 
\begin{lem}\fakelabel{thm:ordunion}
  Let \bbU be a universe.
...
% 
\begin{thm}\fakelabel{thm:wellorder}
...
\begin{thm}\fakelabel{thm:wop}
  ...
  \item For every set $X$, there merely exists a function
    $ f: \powerp X \to X $
    such that $f(Y)\in Y$ for all $Y:\powerp X$.\fakelabel{item:wop1}
  \item Every set merely admits the structure of an ordinal.\fakelabel{item:wop2}
  \end{enumerate}
\end{thm}
% 
% \noindent
...
% \fakelabel{sec:cumulative-hierarchy}
% 
% \index{bargaining|(}%
...
\begin{defn}\fakelabel{defn:V}
  ...
  \item For all $A, B : \UU$, $f : A \to V$ and $g : B \to V$ such that
    %
    \begin{narrowmultline} \fakelabel{eq:V-path}
...
% \begin{equation}
%   \setof{ x | C(x) } \defeq \lam{x}C(x).\fakelabel{eq:class-notation}
% \end{equation}
%
...
% 
\begin{defn}\fakelabel{def:bisimulation}
...
% 
\begin{lem}\fakelabel{lem:BisimEqualsId}
...
% 
\begin{lem}\fakelabel{lem:MonicSetPresent}
...
% \end{proof}
% 
\begin{defn}\fakelabel{def:TypeOfElements}
...
\end{defn}
% 
\begin{thm}\fakelabel{thm:VisCST}
...
% 
\begin{cor}\fakelabel{cor:Delta0sep}
...
% Thus we have shown:
% 
\begin{lem}\fakelabel{lem:fullsep}
...
\sectionExercises
% 
\begin{ex}\fakelabel{ex:utype-ct}
...
\end{ex}
% 
\begin{ex}\fakelabel{ex:add-ordinals}
...
% \end{proof}
% 
\begin{ex}\fakelabel{ex:multiply-ordinals}
...
\end{ex}
% 
\begin{ex}\fakelabel{ex:ninf-ord}
...
\end{ex}
% 
\begin{ex}\fakelabel{ex:well-founded-extensional-simulation}
...
\end{ex}
% 
\begin{ex}\fakelabel{ex:strong-collection}
...
\end{ex}
% 
\begin{ex}\fakelabel{ex:choice-cumulative-hierarchy-choice}
...
\end{ex}
% 
\begin{ex}\fakelabel{ex:plump-ordinals}
...
\end{ex}
% 
\begin{ex}\fakelabel{ex:not-plump}
  Show that \LEM{} is equivalent to the statement ``all ordinals are plump''.
\end{ex}
% 
\begin{ex}\fakelabel{ex:plump-successor}
...
\end{ex}
% 
\begin{ex}\fakelabel{ex:ZF-algebras}
...
\end{ex}
% 
\begin{ex}\fakelabel{ex:monos-are-split-monos-iff-LEM-holds}
