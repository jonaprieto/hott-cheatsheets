...
\begin{defn}\fakelabel{ct:precategory}
...
\begin{defn}\fakelabel{ct:isomorphism}
...
\begin{lem}\fakelabel{ct:isoprop}
...
\begin{lem}[\textsf{idtoiso}]\fakelabel{ct:idtoiso}
...
\begin{eg}\fakelabel{ct:precatset}
  \index{set}%
...
\begin{defn}\fakelabel{ct:category}
  A \define{category}
  \indexdef{category}
...
\begin{lem}\fakelabel{ct:obj-1type}
...
\begin{lem}\fakelabel{ct:idtoiso-trans}
...
\begin{eg}\fakelabel{ct:discrete}
...
\begin{eg}\fakelabel{ct:hoprecat}
...
\begin{eg}\fakelabel{ct:rel}
  ...
  \item If $R(x,y)$ and $S(y',x)$, then $(R\circ S)(y',y)$, and hence $y'=y$.
    Similarly, if $R(x,y)$ and $S(y,x')$, then $x=x'$.\fakelabel{item:rel1}
  \item For any $x$, we have $x=x$, hence $(S\circ R)(x,x)$.
    Thus, there merely exists a $y:Y$ such that $R(x,y)$ and $S(y,x)$.\fakelabel{item:rel2}
  \item Suppose $R(x,y)$.
    ...
    Similarly, $S(y,x) \to R(x,y)$.\fakelabel{item:rel3}
...
\begin{defn}\fakelabel{ct:functor}
  ...
  \item For each $a,b,c:A$ and $f:\hom_A(a,b)$ and $g:\hom_A(b,c)$, we have
    \[\id{F(g\circ f)}{Fg\circ Ff}.\]\fakelabel{ct:functor:comp}
  \end{enumerate}
\end{defn}

% Note that by induction on identity, a functor also preserves \idtoiso.

\begin{defn}\fakelabel{ct:nattrans}
...
\begin{defn}\fakelabel{ct:functor-precat}
...
\begin{thm}\fakelabel{ct:functor-cat}
...
\begin{defn}\fakelabel{ct:functor-composition}
...
\begin{defn}\fakelabel{ct:whisker}
...
\begin{lem}\fakelabel{ct:interchange}
...
\begin{lem}\fakelabel{ct:functor-assoc}
...
\begin{lem}\fakelabel{ct:pentagon}
...
\begin{lem}\fakelabel{ct:adjprop}
...
\begin{lem}\fakelabel{ct:adjointification}
...
\begin{defn}\fakelabel{ct:full-faithful}
...
\begin{defn}\fakelabel{ct:split-essentially-surjective}
...
\begin{lem}\fakelabel{ct:ffeso}
  For any precategories $A$ and $B$ and functor $F:A\to B$, the following types are equivalent.
  \begin{enumerate}
  \item $F$ is an equivalence of precategories.\fakelabel{item:ct:ffeso1}
  \item $F$ is fully faithful and split essentially surjective.\fakelabel{item:ct:ffeso2}
  \end{enumerate}
\end{lem}
% \begin{proof}
...
\begin{defn}\fakelabel{ct:essentially-surjective}
...
\begin{defn}\fakelabel{ct:isocat}
...
\begin{lem}\fakelabel{ct:isoprecat}
  For precategories $A$ and $B$ and $F:A\to B$, the following are equivalent.
  \begin{enumerate}
  \item $F$ is an isomorphism of precategories.\fakelabel{item:ct:ipc1}
  \item There exist $G:B\to A$ and $\eta:1_A = GF$ and $\epsilon:FG=1_B$ such that\fakelabel{item:ct:ipc2}
    \begin{equation}
      \apfunc{(\lam{H} F H)}({\eta}) = \apfunc{(\lam{K} K F)}({\opp\epsilon}).\fakelabel{eq:ct:isoprecattri}
    \end{equation}
  \item There merely exist $G:B\to A$ and $\eta:1_A = GF$ and $\epsilon:FG=1_B$.\fakelabel{item:ct:ipc3}
  \end{enumerate}
\end{lem}
...
\begin{lem}\fakelabel{ct:eqv-levelwise}
...
\begin{lem}\fakelabel{ct:cat-eq-iso}
...
\begin{thm}\fakelabel{ct:cat-2cat}
...
\begin{defn}\fakelabel{ct:opposite-category}
...
\begin{defn}\fakelabel{ct:prod-cat}
...
\begin{lem}\fakelabel{ct:functorexpadj}
...
%
\begin{thm}[The Yoneda lemma]\fakelabel{ct:yoneda}
...
\begin{cor}\fakelabel{ct:yoneda-embedding}
...
\begin{cor}\fakelabel{ct:yoneda-mono}
...
\begin{defn}\fakelabel{ct:representable}
...
\begin{thm}\fakelabel{ct:representable-prop}
...
\begin{lem}\fakelabel{ct:adj-repr}
  For any precategories $A$ and $B$ and a functor $F:A\to B$, the following types are equivalent.
  \begin{enumerate}
  \item $F$ is a left adjoint\index{adjoint!functor}.\fakelabel{item:ct:ar1}
  \item For each $b:B$, the functor $(a \mapsto \hom_B(Fa,b))$ from $A\op$ to \uset is representable\index{representable functor}.\fakelabel{item:ct:ar2}
  \end{enumerate}
\end{lem}
% \begin{proof}
...
\begin{cor}\fakelabel{ct:adjprop2}[\cref{ct:adjprop}]
...
\fakelabel{sec:strict-categories}

% \index{bargaining|(}%

\begin{defn}\fakelabel{ct:strict-category}
...
\begin{eg}\fakelabel{ct:mono-cat}
...
\fakelabel{sec:dagger-categories}
...
\begin{defn}\fakelabel{ct:dagger-precategory}
...
\begin{defn}\fakelabel{ct:unitary}
...
\begin{lem}\fakelabel{ct:idtounitary}
...
\begin{defn}\fakelabel{ct:dagger-category}
...
\begin{eg}\fakelabel{ct:rel-dagger-cat}
...
\begin{defn}\fakelabel{ct:sig}
  ...
  \[ H_{\alpha\beta}(f).\]
    ...
    from $\alpha$ to $\beta$.
  \item For $x:X_0$ and $\alpha:Px$, we have $H_{\alpha\alpha}(1_x)$.\fakelabel{item:sigid}
  \item For $x,y,z:X_0$ and $\alpha:Px$, $\;\beta:Py$, $\;\gamma:Pz$,
if $f:\hom_X(x,y)$ and $g:\hom_X(y,z)$, we have\fakelabel{item:sigcmp}
...
\begin{thm}[Structure identity principle]\fakelabel{thm:sip}
...
\begin{eg}\fakelabel{ct:sip-functor-cat}
...
\begin{defn}\fakelabel{defn:fo-notion-of-structure}
\mbox{}
\begin{enumerate}
...
\begin{lem}\fakelabel{ct:esosurj-postcomp-faithful}
...
\begin{lem}\fakelabel{ct:esofull-precomp-ff}
...
%   \begin{equation}\fakelabel{eq:fullprop}
...
\begin{thm}\fakelabel{ct:cat-weq-eq}
...
%   \item For each $a:A$ and $h:Ha\cong b$, an isomorphism $k_{a,h}:Fa\cong c$; such that\fakelabel{item:eqvprop2}
%   \item For each $(a,h)$ and $(a',h')$ as in~\ref{item:eqvprop2} and each $f:\hom_A(a,a')$ such that $h'\circ Hf = h$, we have $k_{a',h'}\circ Ff = k_{a,h}$.\fakelabel{item:eqvprop3}
%   \end{enumerate}
...
%   \item For each $a:A$ and $h:Ha\cong b$, and each $a':A$ and $h':Ha'\cong b'$, and any $\ell:\hom_A(a,a')$, we have\fakelabel{item:eqvprop5}
...
\begin{thm}\fakelabel{thm:rezk-completion}
...
\begin{eg}\fakelabel{ct:rezk-fundgpd-trunc1}
...
\begin{thm}\fakelabel{ct:weq-iso-precat-cat}
...
% \sectionExercises
%
% \begin{ex}\fakelabel{ex:slice-precategory}
...
% \end{ex}
%
% \begin{ex}\fakelabel{ex:set-slice-over-equiv-functor-category}
...
% \end{ex}
%
% \begin{ex}\fakelabel{ex:functor-equiv-right-adjoint}
...
% \end{ex}
%
% \begin{ex}\fakelabel{ct:groupoids}
...
% \end{ex}
%
% \begin{ex}\fakelabel{ex:2strict-cat}
%   \index{strict!category}%
...
% \end{ex}
%
% \begin{ex}\fakelabel{ex:pre2dagger-cat}
...
% \end{ex}
%
% \begin{ex}\fakelabel{ct:ex:hocat}
...
% \end{ex}
%
% \begin{ex}\fakelabel{ex:dagger-rezk}
...
% \end{ex}
%
% \begin{ex}\fakelabel{ex:rezk-vankampen}
