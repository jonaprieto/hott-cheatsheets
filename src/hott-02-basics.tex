...
% such that
% \begin{equation}\fakelabel{eq:Jconv}
...
\begin{lem}\fakelabel{lem:concat}
...
%   \begin{equation}\fakelabel{eq:concatD}
...
\begin{lem}\fakelabel{thm:omg}%[The $\omega$-groupoid structure of types]
  ...
  \begin{enumerate}
  \item $p= p\ct \refl{y}$ and $p = \refl{x} \ct p$.\fakelabel{item:omg1}
  \item $\opp{p}\ct p=  \refl{y}$ and $p\ct \opp{p}= \refl{x}$.\fakelabel{item:omg2}
  \item $\opp{(\opp{p})}= p$.\fakelabel{item:omg3}
  \item $p\ct (q\ct r)=  (p\ct q)\ct r$.\fakelabel{item:omg4}
  \end{enumerate}
\end{lem}
...
\begin{thm}[Eckmann--Hilton]\fakelabel{thm:EckmannHilton}
...
\begin{defn} \fakelabel{def:pointedtype}
...
\begin{defn} \fakelabel{def:loopspace}
...
\begin{lem}\fakelabel{lem:ap-functor}
  For functions $f:A\to B$ and $g:B\to C$ and paths $p:\id[A]xy$ and $q:\id[A]yz$, we have:
  \begin{enumerate}
  \item $\apfunc f(p\ct q) = \apfunc f(p) \ct \apfunc f(q)$.\fakelabel{item:apfunctor-ct}
  \item $\apfunc f(\opp p) = \opp{\apfunc f (p)}$.\fakelabel{item:apfunctor-opp}
  \item $\apfunc g (\apfunc f(p)) = \apfunc{g\circ f} (p)$.\fakelabel{item:apfunctor-compose}
...
\begin{lem}[Transport]\fakelabel{lem:transport}
...
\begin{lem}[Path lifting property]\fakelabel{thm:path-lifting}
...
\begin{lem}[Dependent map]\fakelabel{lem:mapdep}
...
\begin{lem}\fakelabel{thm:trans-trivial}
...
% \begin{align}
%   \big(f(x) = f(y)\big) &\to \big(\trans{p}{f(x)} = f(y)\big)\fakelabel{eq:ap-to-apd}
\stepcounter{thm}
%   \qquad\text{and} \\
%   \big(\trans{p}{f(x)} = f(y)\big) &\to \big(f(x) = f(y)\big).\fakelabel{eq:apd-to-ap}
\stepcounter{thm}
% \end{align}
...
\begin{lem}\fakelabel{thm:apd-const}
...
\begin{lem}\fakelabel{thm:transport-concat}
  Given $P:A\to\type$ with $p:\id[A]xy$ and $q:\id[A]yz$ while $u:P(x)$, we have
  \[ \trans{q}{\trans{p}{u}} = \trans{(p\ct q)}{u}. \]
\end{lem}

\begin{lem}\fakelabel{thm:transport-compose}
...
\begin{lem}\fakelabel{thm:ap-transport}
...
\fakelabel{sec:basics-equivalences}
%
...
\begin{defn} \fakelabel{defn:homotopy}
...
\begin{lem}\fakelabel{lem:homotopy-props}
...
\begin{lem}\fakelabel{lem:htpy-natural}
...
\begin{cor}\fakelabel{cor:hom-fg}
...
\begin{equation}
  \sm{g:B\to A} \big((f \circ g \htpy \idfunc[B]) \times (g\circ f \htpy \idfunc[A])\big)\fakelabel{eq:qinvtype}
\end{equation}
% is poorly behaved.
...
\begin{defn}\fakelabel{defn:quasi-inverse}
...
\begin{eg}\fakelabel{eg:concatequiv}
...
\begin{eg}\fakelabel{thm:transportequiv}
...
% \begin{enumerate}
% \item For each $f:A\to B$ there is a function $\qinv(f) \to \isequiv (f)$.\fakelabel{item:be1}
% \item Similarly, for each $f$ we have $\isequiv (f) \to \qinv(f)$; thus the two are logically equivalent (see \cref{sec:pat}).\fakelabel{item:be2}
% \item For any two inhabitants $e_1,e_2:\isequiv(f)$ we have $e_1=e_2$.\fakelabel{item:be3}
% \end{enumerate}
...
\begin{equation}\fakelabel{eq:isequiv-invertible}
  \isequiv(f) \;\defeq\;
...
\begin{equation}\fakelabel{eq:eqv}
...
%
\begin{lem}\fakelabel{thm:equiv-eqrel}
...
% \begin{equation}\fakelabel{eq:prodeqv}
...
\fakelabel{sec:compute-cartprod}
%
% \index{type!product|(}%
...
\begin{equation}\fakelabel{eq:path-prod}
  (\id[A\times B]{x}{y}) \to (\id[A]{\proj1(x)}{\proj1(y)}) \times (\id[B]{\proj2(x)}{\proj2(y)}).
\end{equation}
%
\begin{thm}\fakelabel{thm:path-prod}
...
%   \begin{equation}
%     (\id[A]{\proj1(x)}{\proj1(y)}) \times (\id[B]{\proj2(x)}{\proj2(y)}) \to (\id[A\times B]{x}{y}). \fakelabel{eq:path-prod-inverse}
%   \end{equation}
...
%
\begin{thm}\fakelabel{thm:trans-prod}
...
%
\begin{thm}\fakelabel{thm:ap-prod}
...
\begin{thm}\fakelabel{thm:path-sigma}
...
\begin{cor}\fakelabel{thm:eta-sigma}
...
\begin{thm}\fakelabel{transport-Sigma}
...
\begin{equation}
 \eqvspaced{(\id{f}{g})}{\Parens{\prd{x:A} (\id[B(x)]{f(x)}{g(x)})}}.\fakelabel{eq:path-forall}
\end{equation}
...
\begin{align}\fakelabel{eq:transport-arrow}
  \transfib{A\to B}{p}{f} &=
...
\begin{equation}\fakelabel{eq:transport-arrow-families}
\begin{array}{rclcl}
...
%
\begin{lem}\fakelabel{thm:dpath-arrow}
...
%
\begin{lem}\fakelabel{thm:dpath-forall}
...
\fakelabel{sec:compute-universe}
%
% \index{type!universe|(}%
% \index{equivalence|(}%
...
%
\begin{lem}\fakelabel{thm:idtoeqv}
  For types $A,B:\type$, there is a certain function,
  \begin{equation}\fakelabel{eq:uidtoeqv}
...
%
\begin{axiom}[Univalence]\fakelabel{axiom:univalence}
...
%
\begin{lem}\fakelabel{thm:transport-is-ap}
...
%
\begin{thm}\fakelabel{thm:paths-respects-equiv}
...
%
\begin{lem}\fakelabel{cor:transport-path-prepost}
...
\begin{thm}\fakelabel{thm:transport-path}
...
%
\begin{thm}\fakelabel{thm:transport-path2}
...
%
\begin{thm}\fakelabel{thm:dpath-path}
...
% \begin{align}
%   {(\inl(a_1)=\inl(a_2))}&\eqvsym {(a_1=a_2)} \fakelabel{eq:inlinj}\\
%   {(\inr(b_1)=\inr(b_2))}&\eqvsym {(b_1=b_2)}\\
%   {(\inl(a)= \inr(b))} &\eqvsym {\emptyt}. \fakelabel{eq:inlrdj}
% \end{align}
% We prove this as follows.
% Fix an element $a_0:A$; we will characterize the type family
% \begin{equation}
%   (x\mapsto (\inl(a_0)=x)) : A+B \to \type.\fakelabel{eq:sumcodefam}
% \end{equation}
...
\begin{thm}\fakelabel{thm:path-coprod}
...
% \begin{rmk}\fakelabel{rmk:true-neq-false}
...
\begin{thm}\fakelabel{thm:path-nat}
...
\fakelabel{sec:equality-of-structures}
...
% \begin{equation}\fakelabel{eq:transport-semigroup-step1}
%   \begin{aligned}
% {}&a' :     \mathsf{Assoc}(B,m')\\
...
% \begin{equation}
%   \fakelabel{eq:transport-semigroup-assoc}
%   \begin{aligned}
%     m'(m'(b_1,b_2),b_3)
...
% \fakelabel{sec:equality-semigroups}
...
% \begin{equation*} \fakelabel{eq:equality-semigroup-mult}
...
\fakelabel{sec:universal-properties}
...
%   \begin{equation}
%     \lam{x} (\proj1(f(x)),\proj2(f(x))).\fakelabel{eq:prod-ump-rt1}
%   \end{equation}
...
\begin{equation}\fakelabel{eq:prod-umpd-map}
...
\begin{thm}\fakelabel{thm:prod-umpd}
...
%   \begin{equation}
%     \lam{x} (\proj1(f(x)),\proj2(f(x))).\fakelabel{eq:prod-ump-rt2}
%   \end{equation}
...
% \begin{equation}
%   \eqvspaced{\Parens{\prd{w:\sm{x:A} B(x)} C(w)}}{\Parens{\prd{x:A}{y:B(x)} C(x,y)}}.\fakelabel{eq:sigma-lump}
% \end{equation}
...
\begin{equation}
  A\times_C B \defeq \sm{a:A}{b:B} (f(a)=g(b)).\fakelabel{eq:defn-pullback}
\end{equation}
...
%
% \stepcounter{ex}
\begin{ex}\fakelabel{ex:basics:concat}
  Show that the three obvious proofs of \cref{lem:concat} are pairwise equal.
\end{ex}

% \stepcounter{ex}
\begin{ex}\fakelabel{ex:eq-proofs-commute}
...
%
% \stepcounter{ex}
\begin{ex}\fakelabel{ex:ap-to-apd-equiv-apd-to-ap}
...
\end{ex}
%
\begin{ex}\fakelabel{ex:sigma-assoc}
...
\end{ex}
%
\begin{ex}\fakelabel{ex:pullback}
...
\end{ex}
%
\begin{ex}\fakelabel{ex:pullback-pasting}
...
\begin{ex}\fakelabel{ex:eqvboolbool}
  Show that $\eqv{(\eqv\bool\bool)}{\bool}$.
\end{ex}

\begin{ex}\fakelabel{ex:equality-reflection}
...
\begin{ex}\fakelabel{ex:strengthen-transport-is-ap}
...
\begin{ex}\fakelabel{ex:strong-from-weak-funext}
...
\begin{ex}\fakelabel{ex:equiv-functor-types}\
  \begin{enumerate}
...
\begin{ex}\fakelabel{ex:dep-htpy-natural}
